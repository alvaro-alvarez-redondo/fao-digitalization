...
#' @param config named list containing `column_required` as a non-empty character
#' vector of required base column names.
#' @param sheet_names optional character vector of sheet names to read. when
#' provided, workbook sheet discovery is skipped.
...
#' @title split workbook paths into deterministic batches
#' @description splits a character vector of workbook paths into contiguous
#' deterministic batches.
#' @param file_paths character vector of workbook paths.
#' @param batch_size integer scalar batch size greater than or equal to one.
#' @return list of character vectors, one element per batch.
#' @importFrom checkmate check_character check_int
...
#' @title Resolve workbook batch size
#' @description Resolves workbook batch size from centralized constants with
#' optional configuration override.
#' @param config named configuration list.
#' @return integer scalar batch size.
#' @importFrom checkmate check_list check_int
...
#' @title read one workbook batch with per-batch deduplicated reads
#' @description reads workbook paths with per-batch unique-path caching, then
#' reconstructs output in original input order and cardinality.
#' @param file_paths character vector of workbook paths.
#' @param config named configuration list containing `column_required`.
#' @param sheet_names_by_file optional named list mapping workbook paths to
...
#' workbook-level batching with per-batch deduplicated reads. when `future`
#' parallel backends are configured, batches are read in parallel using
#' `future.apply::future_lapply()` for improved throughput on large file sets.
