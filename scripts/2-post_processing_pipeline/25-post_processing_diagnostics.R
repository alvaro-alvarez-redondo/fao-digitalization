...
#' @title ensure post-processing templates exist
#' @description create post-processing templates under `data/exports/templates`
#' when missing so pipeline preflight can run deterministically.
#' @param config named configuration list.
#' @return named list with generated template paths.
#' @importFrom checkmate assert_list
#' @importFrom purrr map_chr
#' @examples
#' \dontrun{ensure_post_processing_templates(config)}
ensure_post_processing_templates <- function(config) {
  checkmate::assert_list(config, min.len = 1)

  cleaning_payloads <- load_cleaning_rules(config)
  harmonization_payloads <- load_harmonization_rules(config)
  numeric_payload <- load_numeric_harmonization_rules(config)

  generated_paths <- c(
    purrr::map_chr(cleaning_payloads, "source_path"),
    purrr::map_chr(harmonization_payloads, "source_path"),
    numeric_payload$source_path
  )

  return(list(
    template_paths = unique(generated_paths)
  ))
}
