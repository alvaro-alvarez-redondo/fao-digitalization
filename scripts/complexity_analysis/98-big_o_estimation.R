...
run_big_o_analysis <- function(
  ...
  if (!quiet) {
    ...
    # Construcción del mensaje adaptativo
    if (hrs > 0) {
      # Formato HH:MM:SS.s si hay al menos 1 hora
      time_str <- sprintf("%02d:%02d:%04.1f (hh:mm:ss)", hrs, mins, secs)
    } else if (mins > 0) {
      # Formato MM:SS.s si hay minutos pero menos de 1 hora
      time_str <- sprintf("%02d:%04.1f (mm:ss)", mins, secs)
    } else {
      # Solo segundos si es menos de 1 minuto
