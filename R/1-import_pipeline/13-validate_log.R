...
#' observations.
#' @param config optional named list containing dataset-specific audit and raw
#' import paths under `paths$data`.
#' @param output_path optional character scalar path for legacy call sites.
#' when provided without `config`, validation writes only the audit workbook and
#' skips raw import mirroring.
...
#' @examples
#' # validate_data(data_example, config = load_pipeline_config("fao_data_raw"))
validate_data <- function(dataset_dt, config = NULL, output_path = NULL) {
  checkmate::assert_data_frame(dataset_dt)

  has_config <- !is.null(config)
  has_output_path <- !is.null(output_path)

  if (!has_config && !has_output_path) {
    cli::cli_abort("either {.arg config} or {.arg output_path} must be provided")
  }
  ...
  resolved_output_path <- if (has_config) {
