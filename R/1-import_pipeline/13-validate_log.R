...
identify_validation_errors <- function(fao_data_raw) {
  ...
  build_character_flags <- function(column_values, allow_missing = FALSE) {
    ...
    is.na(column_values) | trimmed_values == "" | column_values != trimmed_values
  }

  mandatory_flags <- setNames(
    lapply(
      mandatory_character_columns,
      \(column_name) build_character_flags(audit_dt[[column_name]], allow_missing = FALSE)
    ),
    mandatory_character_columns
  )
  ...
  optional_flags <- setNames(
    lapply(
      ...
      \(column_name) build_character_flags(audit_dt[[column_name]], allow_missing = TRUE)
    ),
    optional_character_columns
  )
