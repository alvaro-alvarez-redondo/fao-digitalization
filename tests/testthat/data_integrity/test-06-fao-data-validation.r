...
testthat::test_that("clear_audit_output_directory deletes audit tree and empty parent", {
  temp_root <- withr::local_tempdir()
  audit_parent_dir <- fs::path(temp_root, "audit")
  audit_dir <- fs::path(audit_parent_dir, "dataset_a")
  ...
  testthat::expect_false(fs::dir_exists(audit_parent_dir))
})
...
testthat::test_that(
  "audit_data_output clears stale audit outputs and does not recreate folder when clean",
  {
    ...
    testthat::expect_true(data.table::is.data.table(audited_dt))
    testthat::expect_false(fs::dir_exists(audit_dir))
    testthat::expect_false(fs::dir_exists(fs::path_dir(audit_dir)))
  }
)
