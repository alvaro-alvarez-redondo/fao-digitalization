...
testthat::test_that("export_lists excludes standardized sheet and value/year workbooks", {
  ...
  data_objects <- list(
    demo_raw = data.table::data.table(country = c("a", "b"), value = c("1", "2"), year = c("2020", "2021")),
    demo_cleaned = data.table::data.table(country = c("a", "b"), value = c("1", "2"), year = c("2020", "2021")),
    demo_normalized = data.table::data.table(country = c("a", "b"), value = c("1", "2"), year = c("2020", "2021")),
    demo_harmonized = data.table::data.table(country = c("a", "b"), value = c("1", "2"), year = c("2020", "2021"))
  )
...
testthat::test_that("normalize_for_comparison drops year column", {
  input_dt <- data.table::data.table(year = c("2020", "2021"), value = c("x", "y"))
