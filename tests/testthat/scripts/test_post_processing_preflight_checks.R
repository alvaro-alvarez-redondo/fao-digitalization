testthat::test_that("preflight flags invalid file naming patterns", {
  ...
  withr::with_dir(root_dir, {
    ...
    testthat::expect_true(any(grepl("cleaning stage", result$issues, fixed = TRUE)))
    testthat::expect_true(any(grepl("harmonization stage", result$issues, fixed = TRUE)))
  })
})
...
testthat::test_that("preflight detects column convention mismatch", {
  ...
  withr::with_dir(root_dir, {
    ...
    testthat::expect_true(any(grepl("required columns missing for this run", result$issues, fixed = TRUE)))
  })
})
...
testthat::test_that("assert_post_processing_preflight aborts with stage details", {
  bad_result <- list(
    passed = FALSE,
    issues = c("[cleaning stage] missing file", "[numeric harmonization stage] missing template"),
    checks = list()
  )
...
testthat::test_that("preflight detects auto-run naming mismatch", {
  ...
  withr::with_dir(root_dir, {
    ...
    testthat::expect_false(result$passed)
    testthat::expect_true(any(grepl("auto-run expects", result$issues, fixed = TRUE)))
  })
})


testthat::test_that("ensure_post_processing_templates creates template files automatically", {
  root_dir <- tempfile("fao-preflight-")
  dir.create(root_dir, recursive = TRUE)
  ...
  withr::with_dir(root_dir, {
    ...
    testthat::expect_true(file.exists(file.path(root_dir, "data", "exports", "templates", "cleaning_template.xlsx")))
    testthat::expect_true(file.exists(file.path(root_dir, "data", "exports", "templates", "harmonization_template.xlsx")))
    testthat::expect_true(file.exists(file.path(root_dir, "data", "exports", "templates", "numeric_harmonization_template.xlsx")))
  })
})
